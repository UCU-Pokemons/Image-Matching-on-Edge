...
import alike_step1 as step1_model
...
class SubGraphCompiler:
        ...
        while s + 0.001 >= max(min_scale, min_size / max(H, W)):
            if s - 0.001 <= min(max_scale, max_size / max(H, W)):
                ...
                with torch.no_grad():
                    ...
                    step1 = step1_model.ALike(**configs[args['model']],
                    device=args["device"],
                    top_k=args["top_k"],
                    scores_th=args["scores_th"],
                    n_limit=args["n_limit"])
                    inference_session_step1 = onnxruntime.InferenceSession(
                        "assets/step1.onnx",
                    ...
                    pred = self.predict(step1, model, image)
                    keypoints_ = pred["keypoints"]
                    descriptors_ = pred["descriptors"]
                    scores_ = pred["scores"]

                keypoints.append(torch.tensor(keypoints_))
                descriptors.append(torch.tensor(descriptors_))
                scores.append(torch.tensor(scores_))
            ...
            image = torch.nn.functional.interpolate(torch.tensor(image), (nh, nw), mode='bilinear', align_corners=False)
    ...
    def extract_method(self, m):
        ...
        model = ALike(**configs["alike-t"], device=device, top_k=0, scores_th=0.2, n_limit=5000)

        progbar = tqdm(hpatches, desc='Extracting for {}'.format(m))
        for imgs, homos, seq_name in progbar:
            for i in range(1, 7):
                img = imgs[i - 1]
                pred = self.extract_multiscale(model, img, min_scale=min_scale, max_scale=1, sort=False, n_k=5000)
    ...
    def generate_read_function(self, method, extension='ppm'):
    ...
    def benchmark_features(self, read_feats):
        lim = [1, 5]
        ...
        return i_err, v_err, i_err_homo, v_err_homo
    ...
    def predict(self, step_1, step_2, data):
        ...
        start1 = time.time()
        output = step_1(data.astype(np.float32))
        # output = step_1.run(None, {self.input_node: data.astype(np.float32)})

        time_step1 += time.time() - start1

        start2 = time.time()
        res = step_2(output[0],output[1])
        time_step2 += time.time() - start2


        time_total += time.time() - start1
        return res
    def compile(self):
    ...
    def inference(self):
        ...
        step1 = step1_model.ALike(**configs["alike-t"],
                  device=args["device"],
                  top_k=args["top_k"],
                  scores_th=args["scores_th"],
                  n_limit=args["n_limit"])
        ...
        for inputs in self.data[:5]:
            pred = self.predict(step1, model, inputs)
            kpts = pred["keypoints"]
        ...
        print("Time Total average: " + str(time_total / 5))
        print("Time Step1 average: " + str(time_step1 / 5))
        print("Time Step1 average: " + str(time_step2 / 5))
        time_dkd = model.time_dkd
        time_convertion = model.time_convertion
        time_normalization = model.time_normalization

        print("Time DKD average: " + str(time_dkd / 5))
        print("Time Convertion average: " + str(time_convertion / 5))
        print("Time Normalization average: " + str(time_normalization / 5))

        print("Time NMS: " + str(sum(np.array(model.access_dkd().average_nms)) / 5))


        errors = {}
        for method in methods:
            output_file = os.path.join(cache_dir, method + '.npy')
            read_function = self.generate_read_function(method)
            if os.path.exists(output_file):
                errors[method] = np.load(output_file, allow_pickle=True)
            else:
                self.extract_method(method)
                errors[method] = self.benchmark_features(read_function)
                np.save(output_file, errors[method])
    
        for name, method in zip(names, methods):
            i_err, v_err, i_err_hom, v_err_hom = errors[method]
